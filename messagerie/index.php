...
<tr class='<?php echo $lunonlu; ?>' id='titre_<?php echo $bal['id_bals']; ?>'>
    ...
    <td class='bal_titre_sujet' onclick="afficher_bal('<?php echo $bal['id_bals']; ?>',<?php echo $id_perso;
    echo ", '$page'"; ?>);"><div id='titre_sujet_<?php echo $bal['id_bals']; ?>'><?php echo $bal['titre']; ?></div></td>
    ...
    <td id='bal_<?php echo $bal['id_bals']; ?>' colspan='6' style='display:none;'>
        <div class='bal_outil'>
            <img class='curspointer scroll' onclick="contacter('<?php echo $bal['id_expediteur']; ?>','<?php echo $titre; ?>','<?php echo $reponse_simple; ?>');" src='../images/site/reply.png' title='Répondre'/ >
                <?php if (!$send) {
                    ?><img class='curspointer scroll' onclick="repondre('<?php echo $bal['liste_mats']; ?>','<?php echo $titre; ?>','<?php echo $reponse_tous; ?>');" src='../images/site/reply_all.png' title='Répondre à tous'/ > <?php
                } ?>
                <?php if ($recu) {
                    ?><img class='curspointer' src='../images/site/folder_files.png' title='Archiver'/ onclick="bal_archive('<?php echo $bal['id_bals']; ?>',<?php echo $id_perso; ?>);" /> <?php
                } ?>
            <img class='curspointer' src='../images/site/delete.png' title='Supprimer' onclick="bal_del('<?php echo $bal['id_bals']; ?>',<?php echo $id_perso;
            if ($send) {
                echo ", 'send'";
            } ?>);"/>
        </div>
...
<input class="bal_button" type="button" value="Supprimer" onclick="bal_del_all('<?php echo $id_perso;
if ($send) {
    echo "', 'send";
} ?>');" />
    <?php if ($recu) {
        ?><input class="bal_button" type="button" value="Archiver" onclick="bal_archive_all('<?php echo $id_perso; ?>');" /><?php
    } ?>
    <?php if (!$send) {
        ?><input class="bal_button" type="button" value="Marquer comme lu" onclick="bal_lu_all('<?php echo $id_perso;
if ($send) {
    echo "', 'send'";
} ?>');" /><?php
    } ?>
