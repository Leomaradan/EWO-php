...
<div align="center">
    ...
    <select id="classement_type" name="classement_type">
          <option value="xp" <?php if ($param['classement_type'] == "xp") {
                echo 'selected="selected"';
                             }?>>Chuck Norris (Classement par l'XP)</option>
          <option value="meurtre" <?php if ($param['classement_type'] == "meurtre") {
                echo 'selected="selected"';
                                  }?>>Kaizer Sosey (Classement des tueurs)</option>
          <option value="mort" <?php if ($param['classement_type'] == "mort") {
                echo 'selected="selected"';
                               }?>>Oh mon dieu ! Ils ont tu&eacute; Kenny ! (Classement des tu&eacute;s)</option>
          <option value="cv" <?php if ($param['classement_type'] == "cv") {
                echo 'selected="selected"';
                             }?>>C'&eacute;tait pas ma guerre! (Fait d'armes)</option>
          <option value="survie" <?php if ($param['classement_type'] == "survie") {
                echo 'selected="selected"';
                                 }?>>Il n'en restera qu'un (Classement Survivor)</option>
          <option value="famille" <?php if ($param['classement_type'] == "famille") {
                echo 'selected="selected"';
                                  }?>>Histoire de famille (XP moyenne par familles)</option>
    </select><br/>
    <?php
    ...
    <select id='race' name="race">
            <option value="0" <?php if ($param['race'] == 0) {
                echo 'selected="selected"';
                              }?>>Choix de la race race :</option>
          <option value="0" >Toutes les races</option>
          <option value="1" <?php if ($param['race'] == 1) {
                echo 'selected="selected"';
                            }?>>Humains</option>
          <option value="2" <?php if ($param['race'] == 2) {
                echo 'selected="selected"';
                            }?>>Parias</option>
          <option value="3" <?php if ($param['race'] == 3) {
                echo 'selected="selected"';
                            }?>>Anges</option>
          <option value="4" <?php if ($param['race'] == 4) {
                echo 'selected="selected"';
                            }?>>D&eacute;mons</option>
          <option value="-1" <?php if ($param['race'] == -1) {
                echo 'selected="selected"';
                             }?>>Anges et D&eacute;mons</option>
    </select>

    <select id='grade' name="grade_ord">
            <option value="0" <?php if ($param['grade_ord'] == 0) {
                echo 'selected="selected"';
                              }?>>Classement par Grade :</option>
          <option value="0" >Non</option>
          <option value="1" <?php if ($param['grade_ord'] == 1) {
                echo 'selected="selected"';
                            }?>>Oui, seulement le Grade</option>
          <option value="2" <?php if ($param['grade_ord'] == 2) {
                echo 'selected="selected"';
                            }?>>Oui, Grade et galon</option>
    </select>

    <select id="nb_el" name="nb_el">
            <option value="50" <?php if ($param['nb_el'] == 50) {
                echo 'selected="selected"';
                               }?>>Personnages par page : </option>
          <option value="25" <?php if ($param['nb_el'] == 25) {
                echo 'selected="selected"';
                             }?>>25</option>
          <option value="50" >50</option>
          <option value="100" <?php if ($param['nb_el'] == 100) {
                echo 'selected="selected"';
                              }?>>100</option>
          <option value="150" <?php if ($param['nb_el'] == 150) {
                echo 'selected="selected"';
                              }?>>150</option>
    </select>
    ...
    ?>          
        <table align='center' id="tab_classement" BORDER='0px' CELLPADDING='0'>
            <tr>
                ...
                <?php if (isset($param['afficheGrade'])) {
                    ?><td align="center" class='cla_td_titre large'>Grade</td><?php
                } ?>
                <?php if (isset($param['afficheGalon'])) {
                    ?><td align="center" class='cla_td_titre large'>Galon</td><?php
                } ?>
                <?php if (isset($param['afficheXp']) || isset($param['afficheXpFamille'])) {
                    ?><td align="center" class='cla_td_titre large'>Experience</td><?php
                } ?>
                <?php if (isset($param['afficheMeurtre'])) {
                    ?><td align="center" class='cla_td_titre large'>Meurtre</td><?php
                } ?>
                <?php if (isset($param['afficheMort'])) {
                    ?><td align="center" class='cla_td_titre large'>Mort</td><?php
                } ?>
                <?php if (isset($param['afficheCv'])) {
                    ?><td align="center" class='cla_td_titre large'>Efficacit&eacute;e</td><?php
                } ?>
                <?php if (isset($param['afficheDateMort'])) {
                    ?><td align="center" class='cla_td_titre large'>Derni&egrave;re mort</td><?php
                } ?>
            </tr>
            <?php
            ...
            foreach ($tableau as $rep) {
                ...
                if (isset($param['afficheGrade'])) {
                    echo '<td align="center">' . $rep['grade'] . '</td>';
                }
                if (isset($param['afficheGalon'])) {
                    echo '<td align="center">' . $rep['galon'] . '</td>';
                }
                if (isset($param['afficheXp'])) {
                    echo "<td class='cla_td_exp'>" . round($rep['xp']) . "</td>";
                }
                if (isset($param['afficheXpFamille'])) {
                    echo "<td class='cla_td_exp'>" . round($rep['px']) . "</td>";
                }
                if (isset($param['afficheMeurtre'])) {
                    echo '<td align="center">' . $rep['meurtre'] . '</td>';
                }
                if (isset($param['afficheMort'])) {
                    echo '<td align="center">' . $rep['mort'] . '</td>';
                }
