...
if (!empty($_GET['page'])) {
    $page = mysqli_real_escape_string($conn, max(1, $_GET['page']));
} else {
    $page = 1;
}

if (!empty($_POST['nb_el'])) {
    $nb_el = mysqli_real_escape_string($conn, $_POST['nb_el']);
} elseif (!empty($_GET['nb_el'])) {
    $nb_el = mysqli_real_escape_string($conn, $_GET['nb_el']);
} else {
    $nb_el = 50;
}

if (!empty($_POST['nb_jours'])) {
    $nb_jours = mysqli_real_escape_string($conn, $_POST['nb_jours']);
} elseif (!empty($_GET['nb_jours'])) {
    $nb_jours = mysqli_real_escape_string($conn, $_GET['nb_jours']);
} else {
    $nb_jours = 2;
}
...
if ($grade != -2) {
    if ($grade == 0) {
        ...
        $where_grade = "AND persos.grade_id=$grade";
    }
}


if (!empty($_POST['croissant'])) {
    $croissant = mysqli_real_escape_string($conn, $_POST['croissant']);
} elseif (!empty($_GET['croissant'])) {
    $croissant = mysqli_real_escape_string($conn, $_GET['croissant']);
} else {
    $croissant = "DESC";
}

if (!empty($_POST['jour'])) {
    $time = strtotime(mysqli_real_escape_string($conn, $_POST['jour']));
    $time_m = date('d-m-Y', $time - 24 * 3600);
    $time_p = date('d-m-Y', $time + 24 * 3600);
    $time = date('d-m-Y', $time);
} elseif (!empty($_GET['jour'])) {
    $time = strtotime(mysqli_real_escape_string($conn, $_GET['jour']));
    $time_m = date('d-m-Y', $time - 24 * 3600);
    $time_p = date('d-m-Y', $time + 24 * 3600);
    $time = date('d-m-Y', $time);
} else {
...
<div align="center">
    ...
    <select id='race' name="race">
        <option value="0" <?php if ($race == 0) {
            echo 'selected="selected"';
                          }?>>Choix de la race :</option>
        <option value="0" >Toutes les races</option>
        <option value="1" <?php if ($race == 1) {
            echo 'selected="selected"';
                          }?>>Humains</option>
        <option value="2" <?php if ($race == 2) {
            echo 'selected="selected"';
                          }?>>Parias</option>
        <option value="3" <?php if ($race == 3) {
            echo 'selected="selected"';
                          }?>>Anges</option>
        <option value="4" <?php if ($race == 4) {
            echo 'selected="selected"';
    ...
    <select id='grade' name="grade">
        <option value="-2" <?php if ($grade == -2) {
            echo 'selected="selected"';
                           }?>>Choix du grade :</option>
        <option value="-2" >Tous les grades</option>
        <option value="0" <?php if ($grade == 0) {
            echo 'selected="selected"';
                          }?>>Grade 0 à 3</option>
        <option value="4" <?php if ($grade == 4) {
            echo 'selected="selected"';
                          }?>>Grade 4</option>
        <option value="5" <?php if ($grade == 5) {
            echo 'selected="selected"';
                          }?>>Grade 5</option>
        <option value="-1" <?php if ($grade == -1) {
            echo 'selected="selected"';
                           }?>>Tricheurs</option>
    </select>

    <select id="croissant" name="croissant">
            <option value="DESC" <?php if ($croissant == "DESC") {
                echo 'selected="selected"';
                                 }?>>Ordre :</option>
            <option value="DESC" >Croissant</option>
          <option value="ASC" <?php if ($croissant == "ASC") {
                echo 'selected="selected"';
                              }?>>D&eacute;croissant</option>
    </select>

    <select id="nb_el" name="nb_el">
            <option value="50" <?php if ($nb_el == 50) {
                echo 'selected="selected"';
                               }?>>Personnages par page : </option>
          <option value="25" <?php if ($nb_el == 25) {
                echo 'selected="selected"';
                             }?>>25</option>
          <option value="50" >50</option>
          <option value="100" <?php if ($nb_el == 100) {
                echo 'selected="selected"';
                              }?>>100</option>
          <option value="150" <?php if ($nb_el == 150) {
                echo 'selected="selected"';
                              }?>>150</option>
    </select>
    <select id="nb_jours" name="nb_jours">
            <option value="2" <?php if ($nb_jours == 2) {
                echo 'selected="selected"';
                              }?>>Nombre de jours : </option>
          <option value="1" <?php if ($nb_jours == 1) {
                echo 'selected="selected"';
                            }?>>1</option>
          <option value="2" >2</option>
          <option value="7" <?php if ($nb_jours == 7) {
                echo 'selected="selected"';
                            }?>>7</option>
          <option value="30" <?php if ($nb_jours == 30) {
                echo 'selected="selected"';
                             }?>>30</option>
          <option value="45" <?php if ($nb_jours == 45) {
                echo 'selected="selected"';
                             }?>>45</option>
    </select>
