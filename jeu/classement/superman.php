...
if (isset($_POST['perso_id'])) {
    $id = mysqli_real_escape_string($conn, $_POST['perso_id']);
} elseif (isset($_GET['perso_id'])) {
    $id = mysqli_real_escape_string($conn, $_GET['perso_id']);
} else {
...
for ($inc = 1; $inc <= 4; $inc++) {
    ...
    $res_race = mysqli_query($conn, $sql) or die(mysqli_error($conn));
    while ($rep_race = mysql_fetch_array($res_race)) {
        ...
        $resultat_color = mysqli_query($conn, $sql_color) or die(mysqli_error($conn));
