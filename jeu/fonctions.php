...
function distances($objet1, $objet2, $plan)
{
    ...
    if ($circ[0]) {
        if (abs($objet1['pos_x'] - $objet2['pos_x']) >= ($x_max - $x_min) / 2) {
            ...
            $dist_x = abs($objet1['pos_x'] - $objet2['pos_x']);
        }
    } else {
        $dist_x = abs($objet1['pos_x'] - $objet2['pos_x']);
    }
    ...
    if ($circ[1]) {
        if (abs($objet1['pos_y'] - $objet2['pos_y']) >= ($y_max - $y_min) / 2) {
            ...
            $dist_y = abs($objet1['pos_y'] - $objet2['pos_y']);
        }
    } else {
        $dist_y = abs($objet1['pos_y'] - $objet2['pos_y']);
    }
...
{
    $resultats = mysqli_query($conn, "SELECT consom, poid FROM case_artefact WHERE id = " . $artefact_id . "");
    ...
    if ($poid_new > $poid_portable) {
        ...
        $sql_query = mysqli_query($conn, "INSERT INTO inventaire (id, perso_id, case_artefact_id, statut, pv, consom) VALUES ('' ,'$perso_id', '$artefact_id', 'inactif', '$artefact_pv', '" . $inventaire['consom'] . "')") or die(mysqli_error($conn));
        return true;
    }
}
...
function meurtre($perso_id, $cible_id, $type, $type_esquive = 0)
{
    ...
    if ($perso_race == 1 && ($cible_race == 3 || $cible_race == 4)) {
        if ($cible_grade < 5) {
            ...
            $dif_rang = max(- $perso_rang + $cible_rang + 2, 0);
        };
...
function respawn($id, $type = '', $cible_spawn = '')
{
    if ($type == 'autre') {
        ...
        $resultat_ = mysqli_query($conn, $sql) or die(mysqli_error($conn));
    ...
    switch ($camp) {
        ...
        case 1:
            ...
            while (!$ok) {
                ...
                $research = mysqli_query($conn, $sql);
                $data     = mysqli_fetch_row($research);
                ...
                $research = mysqli_query($conn, '
						...
						WHERE pos_x = ' . $x . '
						AND pos_y = ' . $y . '
						AND carte_id = ' . $data[10]) or die(mysqli_error($conn));
                $data2 = mysqli_fetch_row($research);
        ...
        default:
            ...
            $resultat     = mysqli_query($conn, $sql) or
    ...
    switch ($type_effet) {
        case 'pv':
            ...
            if ($cible[1] == 'allier' || $cible[1] == 'ennemi' || $cible[1] == 'both' || $cible[1] == 'persos') {
                ...
                $cible_caracs_max['pv'] = $cible_carac_noalter['pv_max'];
            }
                        ...
                        if (isset($cible[2]) && ($cible[2] == 'attaque' || $cible[2] == 'attaque_objet')) {
                            ...
                            if ($caracs['force'] % 10 != 0) {
                                ...
                                if ($test <= $reste) {
                                    ...
                                    $malus = max(1, ($caracs['force'] - $caracs['force'] % 10) / 10);
                                }
                            }
                    ...
                    if (($cible_caracs['pv'] + $effet_info['effet']) <= 0 && $cible_caracs['pv'] > 0) {
                        ...
                        if ($cible[1] == 'allier' || $cible[1] == 'ennemi' || $cible[1] == 'both' || $cible[1] == 'persos') {
                            ...
                            if (($cible_caracs['pv'] + $effet_info['effet']) >= $cible_caracs['pv']) {
                                ...
                                calcul_xp($perso_id, $cible[0], 'attaque_' . $cible[1], 1, 0);
                            }
                        }
                    }
                }
        ...
        case 'teleportation':
            if ($cible2[1] != 'porte') {
                ...
                $sql = "SELECT porte_liee_id AS id
					FROM damier_porte
					WHERE id=" . $cible[0];
                $reponse = mysqli_query($conn, $sql) or die(mysqli_error($conn));
        ...
        case 'dla':
            if ($cible[1] == 'allie' || $cible[1] == 'ennemi' || $cible[1] == 'both' || $cible[1] == 'persos') {
                ...
                mysqli_query($conn, "UPDATE persos SET date_tour = '$nouveautour' WHERE id = '" . $cible[0] . "'") or die(mysqli_error($conn));
            }
            break;
        ...
        case 'dissipation': // Dissipe tout les effets magiques
            if ($cible[1] == 'allie' || $cible[1] == 'ennemi' || $cible[1] == 'both' || $cible[1] == 'persos') {
                                mysqli_query($conn, "DELETE FROM caracs_alter_mag   WHERE perso_id = '" . $cible[0] . "' AND cassable = 1") or die(mysqli_error($conn));
            }
            break;
        default:
            if ($cible[1] == 'allie' || $cible[1] == 'ennemi' || $cible[1] == 'both' || $cible[1] == 'persos') {
...
function activ_tour($id, $force_activ = false)
{
    ...
    if ($time >= $datetour || $force_activ) {
        ...
        mysqli_query($conn, "UPDATE caracs_alter SET alter_pa = $alter_pa,
				...
				alter_esq_mag = 0,
				alter_res_mag = 0
				WHERE perso_id = '" . $perso_id . "'") or die(mysqli_error($conn));

                // Décrémentation des effets de sorts
                mysqli_query($conn, "UPDATE caracs_alter_mag SET nb_tour = (nb_tour - 1) WHERE perso_id = '" . $perso_id . "' AND nb_tour > 0");

        //Annulation de tous les effets de sorts de temps 1
        mysqli_query($conn, "DELETE FROM caracs_alter_mag   WHERE perso_id = '" . $perso_id . "' AND nb_tour = 0") or die(mysqli_error($conn));
        ...
        if ($current_pv <= 0) {
            mysqli_query($conn, "UPDATE caracs SET pv = '$max_pv' /2, `force` = '$max_force' /2, perception = '$max_perception' /2 WHERE perso_id = '" . $perso_id . "'") or die(mysqli_error($conn));
            //-- Spawn géré sur le damier.

                        //Annulation de tous les effets de sorts dissipé à la mort
                        mysqli_query($conn, "DELETE FROM caracs_alter_mag WHERE perso_id = '" . $perso_id . "' AND dissipe_mort = 1") or die(mysqli_error($conn));
        } else {
            ...
            mysqli_query($conn, "UPDATE persos SET date_tour = '$nouveautour' WHERE id = '" . $perso_id . "'") or die(mysqli_error($conn));
            //-- Mise a jour des caracs du personnage lors d'un nouveau tour.
            mysqli_query($conn, "UPDATE caracs SET  pv = '$pv', recup_pv = '$max_recup_pv', pa = '$max_pa', pa_dec='$pa_dec', mouv = '$max_mouv', px = '$px', pi = '$pi', malus_def = '$malus_def', `force` = '$max_force', perception = '$max_perception', maj_des=0, res_mag='$res_mag'
					WHERE perso_id = '" . $perso_id . "'") or die(mysqli_error($conn));
