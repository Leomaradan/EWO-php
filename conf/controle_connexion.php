...
// Param�tres de connexion � la base de donn�es
    ...
    if (isset($_SESSION['utilisateur']['droits']) && isset($_SESSION['utilisateur']['droits_date'])) {
        ...
        if ($_SESSION['utilisateur']['droits_date'] > ($time - 3600)) {
            ...
            $sql = "SELECT droits FROM utilisateurs WHERE id = ?";

            $dao->prepare($sql);
            $dao->executePreparedStatement(null, [$utilisateur_id]);
            $connexion = $dao->fetch();
            ...
            $sql1 = "SELECT*FROM utilisateurs_ban WHERE utilisateur_id = ?";

                        $dao->prepare($sql1);
            $dao->executePreparedStatement(null, [$utilisateur_id]);
            $ban_controle = $dao->fetch();
            ...
            if (!empty($ban_controle['utilisateur_id'])) {
                ...
                if ($date_fin < $date_courant) {
                    $dao->prepare("DELETE FROM utilisateurs_ban WHERE utilisateur_id=?");
                     $dao->executePreparedStatement(null, [$utilisateur_id]);
                } else {
