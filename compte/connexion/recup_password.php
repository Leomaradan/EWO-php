...
// Paramètres de connexion à la base de données
    ...
    $sql_users = mysql_fetch_array(mysqli_query($conn, "SELECT nom FROM utilisateurs WHERE email = '$email'"));

    if (!empty($sql_users['nom'])) {
        $sql_users = mysqli_query($conn, "UPDATE utilisateurs SET passwd = '$passencode' WHERE email = '$email'");
        mysqli_close($ewo);
