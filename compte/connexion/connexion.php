...
if (isset($_POST['login']) && isset($_POST['pass'])) {
    $ok = true;
    $password = mysqli_real_escape_string($ewo, $_POST['pass']);
    $utilisateur = mysqli_real_escape_string($ewo, ucfirst(htmlspecialchars(strip_tags($_POST['login']), ENT_COMPAT, 'UTF-8')));
}

if (isset($_GET['login']) && isset($_GET['ticket'])) {
    $ok = true;
    $ticket = mysqli_real_escape_string($ewo, $_GET['ticket']);
    $utilisateur = mysqli_real_escape_string($ewo, ucfirst(htmlspecialchars(strip_tags($_GET['login']), ENT_COMPAT, 'UTF-8')));
}
...
// Paramètres de connexion à la base de données
    ...
    $resultat = mysqli_query($ewo, $sql) or die(mysqli_error($ewo));
    $connexion = mysqli_fetch_array($resultat);
    ...
    $_SESSION['utilisateur']['droits_date'] = $date_courant;
        ...
        $resultat1 = mysqli_query($ewo, $sql1) or die(mysqli_error($ewo));
        $ban_controle = mysqli_fetch_array($resultat1);
    ...
    if (!empty($ban_controle['utilisateur_id'])) {
        ...
        if ($date_fin < $date_courant) {
            mysqli_query($ewo, "DELETE FROM utilisateurs_ban WHERE utilisateur_id='$utilisateur_id'") or die(mysqli_error($ewo));
        } else {
    ...
    //-- Récupération des infos des persos
            ...
            while ($persos = mysqli_fetch_array($resultat)) {
                ...
                $res_pos = mysqli_query($ewo, $sql) or die(mysqli_error($ewo));
                if ($pos = mysqli_fetch_array($res_pos)) {
                ...
                if ($_SESSION['persos']['faction']['id'][$inc]) {
                    ...
                    $res_fac = mysqli_query($ewo, $sql) or die(mysqli_error($ewo));
                    $fac_droits = mysqli_fetch_array($res_fac);
        ...
        //---- Redirection en cas de succés de la connexion.
            ...
            // Le login a réussi, l'utilisateur veux un auto-login
                ...
                mysqli_query($ewo, "REPLACE INTO utilisateurs_ticket (utilisateur_id,ticket,expiration) VALUES ('$utilisateur_id','$ticket','$time')");
            } else {
                // Suppression de la valeur "ticket" dans la bdd
                //mysqli_query($ewo, "UPDATE utilisateurs SET ticket=null WHERE id = '$utilisateur_id' LIMIT 1");
            }
