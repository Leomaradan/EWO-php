...
// r�cup�ration du num�ro de personnage dans la session
...
// R�cup�ration de la race du personnage dans la session
$_SESSION['persos']['race'][$id];
$race = $_SESSION['persos']['race'][$id];

// R�cup�ration du grade du personnage dans la session
$_SESSION['persos']['grade'][$id];
$grade = $_SESSION['persos']['grade'][$id];

$reponse = mysqli_query($conn, "SELECT * FROM caracs WHERE perso_id='" . $_SESSION['persos']['id'][$id] . "'") or die(mysqli_error($conn));
...
include('valeurs.php');//co�ts de base



if ($_POST['pvUp'] > 0) {
    // r�cup�ration du cout du prochain pallier
    $cout_pi = round(($_POST['pvUp']) * ((1 + 0.1 * ($perso_carac['niv_pv'])) * $coutPvBase
    + (1 + 0.1 * ($perso_carac['niv_pv'] + $_POST['pvUp'] - 1)) * $coutPvBase) / 2);
    //echo $cout_pi;
    //v�rification que le perso a suffisament de pi
    if (($new_pi - $cout_pi) >= 0) {
        // si c'est le cas on mets � jour les caracs
...
if ($_POST['forceUp'] > 0) {
    ...
    if (($new_pi - $cout_pi) >= 0) {
            // si c'est le cas on mets � jour les caracs
            //nouvelle valeur du niveau de d�s :
...
if ($_POST['nvMagUp'] > 0 && $_POST['nvMagUp'] + $perso_carac['niv'] <= 5) {
    ...
    while ($i < $_POST['nvMagUp']) {
        ...
        $i++;
            ...
            //v�rification que le perso a suffisament de pi ET que son niveau de magie n'est pas d�j� de 5, car c'est le max possible.
