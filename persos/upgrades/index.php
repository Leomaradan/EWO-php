...
if ($race == 1 || $race == 9) {
    if ($race == 1) {
        $reponse = mysqli_query($conn, 'SELECT COUNT(c.cercle) as nb FROM caracs c
				INNER JOIN persos p ON p.id=c.perso_id
				WHERE
				p.utilisateur_id=' . $utilisateur_id . ' AND
				p.race_id = 1 AND
				(c.cercle IS NOT NULL AND c.cercle != 0);') or die(mysqli_error($conn));
...
if (isset($_POST['choix_cercle']) && isset($_POST['cercle']) && is_numeric($_POST['cercle'])) {
    ...
    } elseif ($_POST['cercle'] > 0 && $_POST['cercle'] < 7 && $affilie && !$donnees['cercle'] && $rep_cercle) {
        maj_carac($perso_id, "cercle", $_POST['cercle']);
    }
...
<div align='center'>
    <?php
    if (isset($admin_mode)) {
        echo "<h2>Simulateur d'ewolution</h2>";
    }
    ?>
    <!-- Debut du coin -->
    <br />
    <div>
        ...
        <table
            ...
            <tr style="border-bottom: 1px solid #dad7b6;">
                <td class="jeu_upgrades" colspan="5"
                    ...
                    <acronym title="Point d'Investissement">PI</acronym> 
                                                               <?php } else { ?>
		...
		<td class="jeu_upgrades"> <span id="upNvMag"><input type="button" value="+" onclick="upCar(7);"/></span';
            ...
            if (
                !isset($admin_mode) && ($affilie || $race == 1 || $race == 2 || $race == 3 || $race == 4
		...
		<option value="10">Technologie : Armement</option>' . $select_end;
                    ...
                    } elseif (($race == 2 || $race == 11) && $donnees['px']) {
                        echo $select_first . '
