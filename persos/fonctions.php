...
function maj_esq_mag($perso_id, $maj, $force_raz)
{
    ...
    $dao->executePreparedStatement($stmt, array('perso_id' => $perso_id));
    $persos = $dao->fetchArray($stmt);
    ...
    if ($maj == 2) {
        ...
        $stmt = $dao->prepare($sql);
        $dao->executePreparedStatement($stmt, array('val' => $val, 'perso_id' => $perso_id));
    }
    ...
    if ($maj && $time >= ($datechangement + 3600 * 12)) {
        if ($maj == 2) {
            ...
            if ($val === 'not(maj_esq_mag)') {
                $dao->exec("UPDATE caracs SET maj_esq_mag=NOT(maj_esq_mag) WHERE perso_id=" . (int) $perso_id);
            } else {
                $sql = "UPDATE caracs SET maj_esq_mag = :val WHERE perso_id = :perso_id";
                $stmt = $dao->prepare($sql);
                $dao->executePreparedStatement($stmt, array('val' => $val, 'perso_id' => $perso_id));
...
function recup_carac($perso_id, $carac_tab)
{
    ...
    if (!isset($perso_id) || !isset($carac_tab) || count($carac_tab) == 0) {
        ...
        foreach ($carac_tab as $colonne) {
            if ($colonne === '*') {
                $colonnes[] = '*';
                continue;
            }

            if (!preg_match('/^[a-zA-Z0-9_]+$/', $colonne)) {
                return false;
            }
...
// if($race!=0){
    ...
    // Exécution/fetch gérés par le DAO.
    // return $reponse['type'];
    // }else return 3;
// }
...
function recup_camp($race)
{
    if ($race != 0) {
        ...
        $stmt = $dao->prepare($sql);
        $dao->executePreparedStatement($stmt, array('race_id' => $race));
        $reponse = $dao->fetchArray($stmt);
...
function recup_camp_plan($camp)
{
    if ($camp != 0) {
        ...
        $stmt = $dao->prepare($sql);
        $dao->executePreparedStatement($stmt, array('camp_id' => $camp));
        $reponse = $dao->fetchArray($stmt);
...
{
    if (!preg_match('/^[a-zA-Z0-9_]+$/', $carac)) {
        return false;
    }
...
{
    if (is_numeric($new_value) && preg_match('/^[a-zA-Z0-9_]+$/', $carac)) {
        $dao = \persos\PersosDAO::getInstance();
        $sql = "UPDATE caracs SET `$carac` = :new_value WHERE perso_id = :perso_id";
        $stmt = $dao->prepare($sql);
        $dao->executePreparedStatement($stmt, array('new_value' => $new_value, 'perso_id' => $perso_id));
    }
}
// Fonction permettant de mettre à jour une carac
...
function maj_carac_alter_mag($perso_id, $carac, $new_value)
{
    if (is_numeric($new_value) && preg_match('/^[a-zA-Z0-9_]+$/', $carac)) {
        ...
        $stmt = $dao->prepare($sql);
        $dao->executePreparedStatement($stmt, array('perso_id' => $perso_id, 'new_value' => $new_value));
    }
}
...
function raz_alter_plan($perso_id)
{
    ...
    $sql = "SELECT 1 FROM caracs_alter_plan WHERE perso_id = :perso_id";
    $stmt = $dao->prepare($sql);
    $dao->executePreparedStatement($stmt, array('perso_id' => $perso_id));

    if (!$dao->fetchRow($stmt)) {
        $sql = "INSERT INTO `caracs_alter_plan` (`perso_id`,
																`alter_pa`,
																`alter_pv`,
																`alter_mouv`,
																`alter_def`,
...
function carac_max($race, $grade, $carac, $niv, $perso_id, $galon = null)
{
    ...
    if (!isset($galon)) {
        $sql_galon = "SELECT `galon_id` AS galon
			FROM `persos`
                WHERE `id`=:perso_id";
        $stmt = $dao->prepare($sql_galon);
        $dao->executePreparedStatement($stmt, array('perso_id' => $perso_id));
        $galon = $dao->fetchArray($stmt);
        $galon = $galon['galon'];
    }
...
function caracs_base_max($id_perso, $race, $grade)
{
    ...
    $sql_caracs = "SELECT `pi`,`niv_pv`, `niv_recup_pv`, `niv_mouv`, `niv_pa`, `niv_des` AS `niv_des`,`niv_force`,`niv_perception`,`niv` AS `magie`
                FROM `caracs`
                    WHERE `perso_id` = :perso_id";
    $stmt = $dao->prepare($sql_caracs);
    $dao->executePreparedStatement($stmt, array('perso_id' => $id_perso));
    $niv = $dao->fetchArray($stmt);
    ...
    if ($old_grade != $grade || $race != $old_race) {
        if ($old_grade > 3 || $grade > 3 || $race != $old_race) {
            ...
            if ($pi_dep < $pi_to_dep) {
                ...
                $new_pi = $niv['pi'] - $diff;
                $sql = "UPDATE caracs SET `pi` = :new_pi WHERE perso_id = :perso_id";
                $stmt = $dao->prepare($sql);
                $dao->executePreparedStatement($stmt, array('new_pi' => $new_pi, 'perso_id' => $id_perso));
            }
            $niv['niv_des'] = $new_niv;
            $sql = "UPDATE caracs SET `niv_des` = :new_niv WHERE perso_id = :perso_id";
            $stmt = $dao->prepare($sql);
            $dao->executePreparedStatement($stmt, array('new_niv' => $new_niv, 'perso_id' => $id_perso));
        }
    }
...
function renew_caracs($id_perso, $new_race, $new_grade)
{
    ...
    $stmt = $dao->prepare($sql);
    $dao->executePreparedStatement($stmt, array('perso_id' => $id_perso));
    $old_caracs = $dao->fetchArray($stmt);
    ...
    $sql = "UPDATE caracs SET
            ...
            WHERE perso_id = :perso_id";
    $stmt = $dao->prepare($sql);
    $dao->executePreparedStatement($stmt, array(
        'pv' => $new_caracs['pv'],
        'pa' => $new_caracs['pa'],
        'mouv' => $new_caracs['mouv'],
        'recup_pv' => $new_caracs['recup_pv'],
        'magie' => $new_caracs['magie'],
        'des' => $new_caracs['des'],
...
{
    $dao = \persos\PersosDAO::getInstance();
    $sql = "UPDATE persos SET superieur_id = 0 WHERE superieur_id = :perso_id";
    $stmt = $dao->prepare($sql);
    $dao->executePreparedStatement($stmt, array('perso_id' => $perso_id));
}
...
function change_race_grade_galon($id_perso, $new_race = null, $new_grade = null, $new_galon = null)
{
    ...
    $sql = "UPDATE persos SET grade_id = :new_grade, race_id = :new_race, galon_id = :new_galon WHERE id = :perso_id";
    $stmt = $dao->prepare($sql);
    $dao->executePreparedStatement($stmt, array(
        'new_grade' => $new_grade,
        'new_race' => $new_race,
...
function calcul_caracs_alter($perso_id = '')
{
    ...
    $nb = 0;
    while ($res_carac_alter_artefact = $dao->fetchArray($stmt)) {
        $nb++;
...
*/
    ...
    $sql = "SELECT race_id, grade_id FROM persos WHERE id = :perso_id";
    $stmt = $dao->prepare($sql);
    $dao->executePreparedStatement($stmt, array('perso_id' => $id_perso));
    $perso = $dao->fetchArray($stmt);

    $sql1 = "SELECT * FROM caracs WHERE perso_id = :perso_id";
    $stmt = $dao->prepare($sql1);
    $dao->executePreparedStatement($stmt, array('perso_id' => $id_perso));
    $force = $dao->fetchArray($stmt);
...
function poid_courant($perso_id)
{
    ...
    $stmt = $dao->prepare($inventaires);
    $dao->executePreparedStatement($stmt, array('perso_id' => $perso_id));
    $objet = $dao->fetchArray($stmt);
    $poid = $objet['poid'];
    if (empty($poid)) {
        $poid = '0';
    }
...
function raz_all()
{
// Exemple legacy: remise a zero des tours / grades / records.
...
// $sql="UPDATE `caracs` SET    `des_attaque`=1,
                            ...
                            // `niv`=0
                            // WHERE perso_id>=0";
// Exécution legacy de la requête ci-dessus.
