...
switch ($act) {
    ...
    case 'rmorder':
        ...
        die(mysqli_error($conn) . "[$sql]" . __FILE__ . "@" . __LINE__);
    ...
    while (($tbl = mysql_fetch_assoc($Rres))) {
        ...
        if ($matcible == 0) {
            ...
            $res = mysqli_query($conn, $sql) or die(mysqli_error($conn) . "[$sql]" . __FILE__ . "@" . __LINE__);
...
?>
    ...
    <?php
      ...
      $res = mysqli_query($conn, $sqlreq) or die(mysqli_error($conn) . "[$sqlreq]");
    while ($rw = mysql_fetch_assoc($res)) {
        $sqlreq = "SELECT * from ia_orders WHERE matricule=" . $rw['id'] . " ORDER BY priorite DESC";
        $ures = mysqli_query($conn, $sqlreq) or die(mysqli_error($conn) . "[$sqlreq]");
...
//        ."<td><a href='main.php?act=summon&id=".$rw["id"]. "'>Invoquer</a></td>"
    ...
    <?php
         ...
         $rep = mysqli_query($conn, $sqlr) or die(mysqli_error($conn) . "[$sqlr]" . __FILE__ . "@" . __LINE__);
