...
function poper_pnj($pnjid, $creator)
{
      ...
      $sql = mysqli_query($conn, $qry) or die(mysqli_error($conn) . "[$qry]");
...
function create_pnj($perso_nom, $pi, $creator, $typeia, $typearch)
{
    ...
    $resultat = mysql_query ($sql) or die (mysqli_error($conn)."[$sql]");
    ...
    $resultat = mysqli_query($conn, $sql) or die(mysqli_error($conn) . "[$sql]");
    ...
    $perso_bg = "ou pas";
        ...
        $verif_nom_existe = mysqli_query($conn, "SELECT nom FROM persos WHERE nom = '$perso_nom'") or die("Erro checking name $perso_nom" . mysqli_error($conn));
    if (mysqli_fetch_row($verif_nom_existe)) {
          return false;
    }
    ...
    $sql_perso = mysqli_query($conn, "INSERT INTO persos(
												id, 
												background, 
												description_affil, 
												utilisateur_id, 
    ...
    $sql_carac_alter = mysqli_query($conn, "INSERT INTO `caracs_alter` (`perso_id`, 
																`alter_pa`, 
																`alter_mouv`, 
																`alter_def`, 
																`alter_att`, 
    ...
    /*$sql_carac_alter_mag = mysqli_query($conn, "INSERT INTO `caracs_alter_mag` (`perso_id`,
                                                                `alter_pa`,
                                                                `alter_mouv`,
                                                                `alter_def`,
                                                                `alter_att`,
    ...
    $sql_perso_carac = mysqli_query($conn, "INSERT INTO `caracs` (	`perso_id`, 
															`px`, 
															`pi`, 
															`pv`, 
															`recup_pv`, 
        ...
        $sql_perso_design = mysqli_query($conn, "INSERT INTO `blocks` (`unique_id`, `perso_id`, `block_id`, `column_id`, `order_id`) VALUES
    ...
    if ($sql_perso_design == false) {
        ...
        mysqli_query($conn, $qr) or die("toto [ $qr ] " . mysqli_error($conn));
