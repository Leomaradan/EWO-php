            ...
            if (move_uploaded_file($_FILES['fichier']['tmp_name'], $target . $prefix . $_FILES['fichier']['name'])) {
              // Si upload OK
                ...
                $avatarsql = "SELECT avatar_url FROM persos WHERE id = $id_perso";
                            $resultatavat = mysqli_query($conn, $avatarsql) or die(mysqli_error($conn));
                ...
                $sql = mysqli_query($conn, "UPDATE persos SET avatar_url = '$cible' WHERE utilisateur_id = $utilisateur_id AND id = $id_perso");
