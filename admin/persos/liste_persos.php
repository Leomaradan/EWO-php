...
if (!isset($_GET['alpha'])){
    ...
    $alpha = mysqli_real_escape_string($conn, $_GET['alpha']);
}
?>
...
for ($i='A';$i!='AA';$i++){
    ...
    $resultat = mysql_query ($count) or die (mysqli_error($conn));
...
<table id="liste_persos" class="tablesorter"> 
    ...
    <tbody>
        ...
        while ($perso = mysql_fetch_array($resultat)) {
            ...
            if ($perso['nom_race'] != null) {
                $special[] = 'Race personnalisée';
            }
            if ($perso['pnj'] != 0) {
                $special[] = 'PNJ';
            }
...
<ul>
<?php
    /*$resultat = mysql_query ($persos) or die (mysqli_error($conn));
    while ($perso = mysql_fetch_array ($resultat)){
